...
import { Phone, Mail, MapPin, Calendar, Search, Filter, UserX, Eye, CreditCard } from "lucide-react"
...
export default function EmployeesPage() {
  ...
  const confirmFireEmployee = () => {
    ...
    console.log("Firing employee:", selectedEmployee?.firstName, "Reason:", fireReason, "Details:", fireDetails)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage your current household staff</p>
        </div>
        <Button className="bg-green-600 hover:bg-green-700">Add New Employee</Button>
      </div>
            ...
            <CardHeader className="pb-4">
              <div className="flex items-center space-x-4">
                ...
                <div className="flex-1">
                  ...
                  <Badge className="bg-green-100 text-green-800 hover:bg-green-100 mt-1">{employee.status}</Badge>
                </div>
              </div>
            </CardHeader>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="truncate">{employee.vettedAddress.split(",")[0]}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Calendar className="h-4 w-4 text-gray-400" />
                  <span>{new Date(employee.dateOfEmployment).toLocaleDateString()}</span>
                </div>
              </div>

              <div className="text-sm">
                <p className="text-gray-600 mb-1">Salary: ₦{employee.salary.toLocaleString()}/month</p>
                <p className="text-gray-600">Rating: {employee.rating}/5 ⭐</p>
              </div>

              <div className="flex gap-2 pt-2">
                <Button variant="outline" size="sm" className="flex-1" onClick={() => handleViewEmployee(employee)}>
      ...
      <Dialog open={!!selectedEmployee && !showFireDialog} onOpenChange={() => setSelectedEmployee(null)}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-lg text-green-600 font-medium">{selectedEmployee.position}</p>
                  <Badge className="bg-green-100 text-green-800 hover:bg-green-100">{selectedEmployee.status}</Badge>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      <Phone className="h-4 w-4 text-gray-400" />
                      <span>{selectedEmployee.alternativeNumber}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <Mail className="h-4 w-4 text-gray-400" />
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <span>{selectedEmployee.vettedAddress}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      <Calendar className="h-4 w-4 text-gray-400" />
                      <span>Started: {new Date(selectedEmployee.dateOfEmployment).toLocaleDateString()}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <CreditCard className="h-4 w-4 text-gray-400" />
                      <span>Salary: ₦{selectedEmployee.salary.toLocaleString()}/month</span>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Experience: {selectedEmployee.experience}</p>
                      <p className="text-sm text-gray-600">Rating: {selectedEmployee.rating}/5 ⭐</p>
                    </div>
                    {selectedEmployee.driversLicenseId !== "N/A" && (
                      <div>
                        <p className="text-sm text-gray-600">Driver's License: {selectedEmployee.driversLicenseId}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <h4 className="font-semibold text-gray-900">Guarantor Information</h4>
                <div className="space-y-2">
                  {selectedEmployee.guarantorNames.map((name: string, index: number) => (
                  ...
                  <div className="flex items-start gap-3">
                    <MapPin className="h-4 w-4 text-gray-400 mt-1" />
                    <span className="text-gray-700">{selectedEmployee.guarantorAddress}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={() => handleFireEmployee(selectedEmployee)}>
      ...
      <Dialog open={showFireDialog} onOpenChange={setShowFireDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-red-600">Fire Employee</DialogTitle>
          </DialogHeader>

          <div className="space-y-4">
            <p className="text-gray-600">
              Are you sure you want to terminate {selectedEmployee?.firstName} {selectedEmployee?.lastName}? This action
              cannot be undone.
            </p>
            ...
            <div className="space-y-2">
              ...
              <Select value={fireReason} onValueChange={setFireReason}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="redundancy">Position Redundancy</SelectItem>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowFireDialog(false)}>
              Cancel
            </Button>
            <Button variant="destructive" onClick={confirmFireEmployee} disabled={!fireReason}>
              Confirm Termination
            </Button>
          </DialogFooter>
        </DialogContent>
